...
if [ $ERR=0 ]; then
  ...
  if [ $CODE=200 ]; then
    ...
    sudo docker tag jenkins_identidock dockerhost1:5000/identidock:$HASH
    sudo docker tag jenkins_identidock dockerhost1:5000/identidock:newest
    echo "Pushing"
    sudo docker push dockerhost1:5000/identidock:$HASH
    sudo docker push dockerhost1:5000/identidock:newest
  else  
